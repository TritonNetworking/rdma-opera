...
void print_parameters(struct dccs_parameters *params) {
    ...
    log_info("Config: verb = %s, count = %zu, length = %zu, server = %s, port = %s, exec = %s.\n", verb, params->count, params->length, params->server, params->port, params->exec);
...
#define OPT_TOS 1003
        ...
        c = getopt_long(argc, argv, "b:c:v:p:m:r:w:e:Vh", long_options, NULL);
        if (c == -1)
            break;
    ...
    dccs_validate(check_exec(params->exec) == 0, argv, "exec must be an executable.\n");
